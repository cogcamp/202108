...
mainScene.create = function() {
    ...
    // スペースキーのクリックでボール発射
    this.input.keyboard.on('keydown-SPACE', function(ebent){
        //ゲーム開始状態ならば
...
mainScene.config = function() {
    // 背景色の設定
    this.cameras.main.setBackgroundColor('#fffccc');
...
mainScene.createBall = function() {
    // ボール作成
    this.ball = this.physics.add.image(380, 500, 'ball1');
...
mainScene.createPaddle = function() {
     // パドル作成
    this.paddle = this.physics.add.image(400, 50, 'paddle1');
    this.paddle2 = this.physics.add.image(400, 550, 'paddle2');
    this.paddle.setDisplaySize(104,24);
    this.paddle2.setDisplaySize(208, 48);
    this.paddle.setImmovable();
    this.paddle2.setImmovable();
    this.paddle.isStart = true;
    this.paddle2.isStart = true;
    this.physics.add.collider(this.paddle, this.ball, this.hitPaddle, null, this);
    this.physics.add.collider(this.paddle2, this.ball, this.hitPaddle2, null, this);
};

mainScene.hitPaddle = function (paddle, paddle2, ball) {
    // ボールにX方向の角度を設定
    var diff = 0;
    if(ball.x < paddle.x){
        //ボールがパドルの左側に衝突
     ...
     if(ball.x < paddle2.x){
        //ボールがパドルの左側に衝突
        diff = paddle2.x - ball.x;
        ball.setVelocityX(-10 * diff);
    }else if(ball.x > paddle2.x){
        //ボールがパドルの右側に衝突
        diff = ball.x - paddle2.x;
        ball.setVelocityX(10 * diff);
    }else{
        //X方向の加速度なし
        ball.setVelocityX(0);
...
mainScene.createBlocks = function() {
    // 横10列、縦6行並べる
    //ブロックの色の配列
    var blockColors = ['red1', 'green1', 'yellow1', 'silver1', 'blue1', 'purple1', 'red2', 'green2', 'yellow2', 'silver2', 'blue2', 'purple2'];
