...
mainScene.createBlocks = function() {
    // 横10列、縦6行並べる
    //ブロックの色の配
    var blockColors = ['purple1','purple2','red1','silver1','yellow1','green1','red2','silver2','yellow2','green2','blue1','blue2'];
    ...
    for(var i = 0; i < 80; i++ ){
       ...
       var colorNo = Math.floor( Math.random() * 12);
        var color = blockColors[colorNo];
        var block = this.blocks.create(80 + x * 64, 80 + y * 32, color);
        block.setOrigin(0,0);
...
//    }
    ...
    this.physics.add.collider(this.ball, this.blocks, this.hitBlock,
     null, this);
    };
...
mainScene.gameOver = function() {
    // ゲームオーバー
    alert("ゲームオーバー   "  +  "もう一度やってみよう      " + " ヒント ボールをよく見てバーに当たるところを予想してスピードと飛ぶところを調整する");
    // スタートシーンに移動
    this.scene.start("startScene");
};
